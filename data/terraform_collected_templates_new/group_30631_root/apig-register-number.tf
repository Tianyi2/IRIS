...
resource "aws_lambda_permission" "register_number_post" {
  ...
  source_arn = "arn:aws:execute-api:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:${aws_api_gateway_rest_api.messaging[0].id}/*/${each.value.http_method}${aws_api_gateway_resource.register_number[each.key].path}"
}
