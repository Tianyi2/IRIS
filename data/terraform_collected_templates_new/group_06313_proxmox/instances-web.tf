...
locals {
  ...
  webs = { for inx, zone in local.zones : zone => inx if lookup(try(var.instances[zone], {}), "enabled", false) && lookup(try(var.instances[zone], {}), "web_count", 0) > 0 }
}
...
module "webs" {
  ...
  vms         = lookup(try(var.instances[each.key], {}), "web_count", 0)
  id          = lookup(try(var.instances[each.key], {}), "web_id", 9000)
  name        = "${local.web_prefix}-${format("%02d", index(local.zones, each.key))}"
  description = "Web node managed by Terraform"
  cpus        = lookup(try(var.instances[each.key], {}), "web_cpu", 1)
  memory      = lookup(try(var.instances[each.key], {}), "web_mem", 2048)
  hugepages   = 1024
  ...
  network = {
    "vmbr0" = {
      firewall        = lookup(var.security_groups, "web", "") == "" ? false : true
      firewall_groups = lookup(var.security_groups, "web", "")
      ip6             = ""
      ip6subnet       = cidrsubnet(lookup(try(var.nodes[each.key], {}), "ip6", "fe80::/64"), 16, 1 + index(local.zones, each.key))
      ip6mask         = 64
      ip6index        = 256 + lookup(try(var.instances[each.key], {}), "web_id", 9000)
