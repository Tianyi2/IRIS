...
locals {
  ...
  hub1_client_addr     = cidrhost(local.hub1_subnets["MainSubnet"].address_prefixes[0], 4)
  hub1_server_addr     = cidrhost(local.hub1_subnets["MainSubnet"].address_prefixes[0], 5)
  hub1_appgw_addr      = cidrhost(local.hub1_subnets["AppGatewaySubnet"].address_prefixes[0], 99)
  hub1_client_hostname = "clientVm"
  hub1_server_hostname = "serverVm"
}
